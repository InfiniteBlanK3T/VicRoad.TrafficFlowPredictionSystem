...
def format_prediction_result(street: str, date: str, time: str, prediction: float, actual: float) -> str:
...
def plot_prediction(ax: plt.Axes, street_data: pd.DataFrame, prediction: float, actual: float, selected_time: str):
    ...
    ax.scatter(selected_time, prediction, color='red', s=100, zorder=5, label='Prediction')
    ax.scatter(selected_time, actual, color='green', s=100, zorder=5, label='Actual (selected)')
    ...
    ax.set_xticks(times[::12])  # Show every 3 hours
